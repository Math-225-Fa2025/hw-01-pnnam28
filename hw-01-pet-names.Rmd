...
- ANSWERS: NA, Lucy, and Charlie. Suprisingly, the most named pets are left NA which mean most of the pet are no name. 
CODE: 
seattlepets %>%
...
+     arrange(species, n) 
```
ANSWER: compare the arrange(species,n) and arrange(n,species). The aragument arrange(species,n) will be order the sort the species by alphabet and then will be follow by the n total number of names. When the arrange (n,species) will be sort the number of name first and then will be the species by alphabet order. 
...
- ANSWER : Lily, Luna, Oliver, Max. Those names near the line will be most popular for both species but the further away from x-axis will be more popular for cat and same behaviour for dogs where will be from y-axis.
...
- ANSWER: The relationship between the two variables appears to be independent. However, there is a noticeable trend in dog names compared to cat names. It seems that dogs have a wider variety of names than cats, which could suggest that dogs are more popular as pets in the area, or that many cat owners simply choose not to name their pets.
